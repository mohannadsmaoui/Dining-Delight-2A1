...
*/	
   ...
   echo " Les datas : " ;
	...
	function valableProduit($id,$valable)
		  {

			$sql="UPDATE produit SET available=:valable where id_p=:id";
			$db = config::getConnexion();
			$req=$db->prepare($sql);
			$req->bindValue(':valable',$valable);
			$req->bindValue(':id',$id);
			$req=$req->execute();
...
function modifierCat ($id_cat,$id_p)
{

	$sql="UPDATE produit SET id_cat=:id_cat WHERE id_p=:id_p_org";
	$db = config::getConnexion();
	$req=$db->prepare($sql);
	

        $req->bindValue(':id_cat',$id_cat);
		$req->bindValue(':id_p_org',$id_p);
